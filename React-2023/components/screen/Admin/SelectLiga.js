...
import {  SafeAreaView, Text, View,Image, TouchableOpacity, ScrollView,FlatList, Pressable } from 'react-native';
...
import {db,firebase,auth,admin} from './../Services/firebaseconfig.js';
...
const teste = () => {
  ...
  useEffect(() => {
    if (collection2.length > 0 && selectedButton === 1) {
      SetDonthave(false);
    } else if (collection3.length > 0 && selectedButton === 2) {
      SetDonthave(false);
    } else if (collection4.length > 0 && selectedButton === 3) {
      SetDonthave(false);
    } else {
      SetDonthave(true);
    }
  }, [collection2, collection3, collection4, selectedButton,SetDonthave]);
  ...
  return (
    ...
    <View style={[styles.conteiner,{flex:1}]}>
        <Pressable style={{ right: -8 }}>
          <View>
            <FontAwesome name="angle-left" size={28} color="black" onPress={() => navigation.goBack()} />
          </View>
        </Pressable>
            ...
            <View style={{backgroundColor:'silver', flexDirection:'row', justifyContent:'space-evenly', alignItems: 'center', margin: 10, borderRadius: 10}}>
                {/* Imagem liga */}
                {jogo.length > 0 && (
                    <Image style={{width:50, height:55, marginLeft: '-30%', margin:10}} source={{ uri: jogo[0].league.logo }}/>
                )}
                {/* Nome Liga */}
                <View>
                    <Text>
                        {jogo.length > 0 && jogo[0].league.name}
                    </Text>
                </View>                
            </View>         
        {/* BOtao adicionar */}
        <View style={styles.Co}>
          <View style={styles.btnContainerAdc}>
            <TouchableOpacity style={styles.btnBestAdc}>
    ...
    {Game2 == 1 ?(
      ...
      {Game3 == 1 ?(
              ...
              {Donthave ?(
                     ...
                     <View style={{flex:1}}>
                   <Image style={{ width: 280, height: 280, marginBottom: 10, alignSelf: 'center' , top:-40}} source={require('./../../../assets/donthavegames.gif')} />
